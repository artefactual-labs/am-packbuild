...
curl -fsSL https://packages.archivematica.org/1.19.x/key.asc | sudo gpg --dearmor -o /etc/apt/keyrings/archivematica-1.19.x.gpg
sudo sh -c 'echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/archivematica-1.19.x.gpg] http://packages.archivematica.org/1.19.x/ubuntu-externals jammy main" > /etc/apt/sources.list.d/archivematica-externals.list'
...
EOF'
    ...
    sudo sh -c 'echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/archivematica-1.19.x.gpg] http://packages.archivematica.org/1.19.x/ubuntu jammy main" > /etc/apt/sources.list.d/archivematica.list'
fi
