...
# Reuse the jammy ubuntu-externals packages until noble equivalents are available.
...
deb file:/am-packbuild/debs/noble/_deb_repository ./
EOF'
else
    sudo sh -c 'echo "deb [arch=amd64 signed-by=/etc/apt/keyrings/archivematica-1.18.x.gpg] http://packages.archivematica.org/1.18.x/ubuntu noble main" > /etc/apt/sources.list.d/archivematica.list'
fi
